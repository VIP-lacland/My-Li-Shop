...
if(isset($_POST['search']))
{
    ...
    try {
        // Chuyển sang PDO Prepared Statement
        $sql = "SELECT id,image,name,price FROM products WHERE (name LIKE :keyword)";
        $stmt = $conn->prepare($sql);
        $searchPattern = "%" . $searchKeyword . "%";
        $stmt->bindParam(':keyword', $searchPattern, PDO::PARAM_STR);
        $stmt->execute();
        
        $resultSearch = $stmt;
        $totalnumber = $stmt->rowCount();
    ...
    <div class="container-fluid">
        <div id="bg">
            ...
            <?php 
                    ...
                    echo "Lỗi truy vấn slides: " . $e->getMessage();
                }
            ?>
        </div>
    </div><!-- /background -->
                        ...
                        <div class="content-product-main">
                            <div class="row">
                                <?php
                                    $i = 0;
                                    if ($resultSearch) {
                                        while ($kq = $resultSearch->fetch(PDO::FETCH_ASSOC))
                                        {
                                            $i++;
                                ?>                        
